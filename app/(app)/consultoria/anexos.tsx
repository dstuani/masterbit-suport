...
import { enviarAnexoTopicoAction, removerAnexoTopicoAction } from "./actions";
...
/**
 * Envio de anexos do tópico: botão, arrastar e soltar, e Ctrl+V para prints.
 * Mesmo padrão dos anexos de atendimento — só troca o parâmetro e a action.
 */
export function EnviarAnexosDoTopico({ topicoId }: { topicoId: string }) {
  ...
  function enviar(arquivos: File[]) {
    ...
    iniciar(async () => {
      ...
      for (const original of arquivos) {
        ...
        dados.set("topico_id", topicoId);
        dados.set("arquivo", arquivo);

        try {
          const resultado = await enviarAnexoTopicoAction(dados);
...
export function RemoverAnexoDoTopico({ anexoId, nome }: { anexoId: string; nome: string }) {
  const [pendente, iniciar] = useTransition();
  ...
  function remover() {
    ...
    iniciar(async () => {
      ...
      const resultado = await removerAnexoTopicoAction(dados);
